...
Page({
  ...
  citySure: function (e) {
    ...
    if (this.data.areas[value[0]].name == '绥棱县') {
      var city = '绥化市'
      that.setData({
        city: city,
        areaInfo: "黑龙江省 " + city + " " + areaInfo,
      })
    }
    else{
       that.setData({
  ...
  gotoRecycle() {
    this.save();
    wx.navigateTo({
      url: '../recycle/recycle?orderKind=fc2',
    })
  },
  gotoUnRecycle() {
    this.save();
    wx.navigateTo({
      url: '../unRecycle/unRecycle?orderKind=fc2',
    })
  },
  gotoSoil() {
    this.save();
    wx.navigateTo({
      url: '../soil/soil?orderKind=fc2',
    })
  },
  ...
  submit() {
    if (this.data.name == '' || this.data.tele == '' || this.data.areaInfo == '') {
      wx.showToast({
        title: '请完善发起人信息',
        icon: 'none',
        duration: 1000,
      ...
      var link = app.globalData.http + '/api/cleaner/record/FCOrder';
      var Token = wx.getStorageSync('token');
      ...
      wx.request({
        ...
        success(res) {
          ...
          if (res.data.code == 200) {
            ...
            app.globalData.fcOrder.garbageChooses = [];
            wx.showToast({
              title: '提交成功',
              icon: 'success',
              duration: 2000,
  ...
  onLoad: function (options) {
    if (app.globalData.order.name != null) {
      this.setData({
        ...
        city: app.globalData.order.city,
        ...
        price:app.globalData.fcOrder.price,
